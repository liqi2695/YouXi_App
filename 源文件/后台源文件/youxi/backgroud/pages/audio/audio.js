...
layui.use(['table', 'jquery', 'form', 'admin'], function () {
	...
	table.render({
		...
		cols: [
			[{
				...
				field: 'audioid', title: 'ID', sort: true
			},{
				field: 'PersonalNickName', title: '用户名', templet: '#usernameTpl'
			},{
				field: 'PersonalPhone', title: '手机', templet: '#usernameTpl'
			},{
                field: 'Position', title: '地址', templet: '#usernameTpl'
            },{
				field: 'Time', title: '发布时间', sort: true
            },{
                field: 'VideosTitle', title: '视频名称', templet: '#usernameTpl'
            },{
                field: 'RandomPictureUrl', title: '戏剧封面地址', templet: '#usernameTpl'
            },{
	...
	$('#search').click(function () {
		...
		var Phone = $('#phone').val();
	     var date = $('#time').val();
	if($('#user').val()==""&&$('#time').val()==""&&$('#phone').val()==""){
		layer.msg('查询内容不能为空');
		return false;
	}
...
    
	...
	window.member_edit = function (obj,id) {
		...
		WeAdminShow('编辑音频资料', './edit.html', 500, 400);
	}
    //查看
    window.member_show = function (obj,id) {
		...
		WeAdminShow('查看音频资料', './show.html', 500, 400);
	}
	/*用户-删除*/
